...
public class ModMultiplayerServerListWidget extends AlwaysSelectedEntryListWidget<MultiplayerServerListWidget.Entry> {
    ...
    private final ModMultiplayerScreen screen;
    private final List<ModMultiplayerServerListWidget.ServerEntry> servers = Lists.newArrayList();
    ...
    public ModMultiplayerServerListWidget(ModMultiplayerScreen modMultiplayerScreen,MinecraftClient minecraftClient, int i, int j, int k, int l) {
        super(minecraftClient, i, j, k, l);
        screen = modMultiplayerScreen;
    }
    ...
    public void setSelected(@Nullable ModMultiplayerServerListWidget.Entry entry) {
    ...
    public void setServers(ServerList servers) {
        ...
        for (int i = 0; i < servers.size(); i++) {
            this.servers.add(new ModMultiplayerServerListWidget.ServerEntry(screen, servers.get(i)));
        }
...
//        }
    ...
    public static class ScanningEntry extends MultiplayerServerListWidget.Entry {
        ...
        public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            ...
            context.drawText(
                    ...
                    ModMultiplayerServerListWidget.LAN_SCANNING_TEXT,
                    this.client.currentScreen.width / 2 - this.client.textRenderer.getWidth(ModMultiplayerServerListWidget.LAN_SCANNING_TEXT) / 2,
                    i,
                    16777215,
                    false
            );
        ...
        @Override
        public Text getNarration() {
            return ModMultiplayerServerListWidget.LAN_SCANNING_TEXT;
        }
    }
    ...
    public class ServerEntry extends MultiplayerServerListWidget.Entry {
        ...
        protected ServerEntry(final ModMultiplayerScreen screen, final ServerInfo server) {
            this.screen = screen;
            this.server = server;
            this.client = MinecraftClient.getInstance();
        ...
        public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickDelta) {
            if (this.server.getStatus() == ServerInfo.Status.INITIAL) {
                ...
                ModMultiplayerServerListWidget.SERVER_PINGER_THREAD_POOL
                        .submit(
                                () -> {
                                    try {
                                        this.screen
                                        ...
                                        this.server.label = ModMultiplayerServerListWidget.CANNOT_RESOLVE_TEXT;
                                        this.client.execute(this::update);
                                    } catch (Exception exception) {
                                        this.server.setStatus(ServerInfo.Status.UNREACHABLE);
                                        this.server.label = ModMultiplayerServerListWidget.CANNOT_CONNECT_TEXT;
                                        this.client.execute(this::update);
                                    }
                                }
                        );
            ...
            if (this.server.getStatus() == ServerInfo.Status.PINGING) {
                ...
                this.statusIconTexture = switch (i) {
                    case 1 -> ModMultiplayerServerListWidget.PINGING_2_TEXTURE;
                    case 2 -> ModMultiplayerServerListWidget.PINGING_3_TEXTURE;
                    case 3 -> ModMultiplayerServerListWidget.PINGING_4_TEXTURE;
                    case 4 -> ModMultiplayerServerListWidget.PINGING_5_TEXTURE;
                    default -> ModMultiplayerServerListWidget.PINGING_1_TEXTURE;
                };
            }
            ...
            if (this.client.options.getTouchscreen().getValue() || hovered) {
                ...
                if (this.canConnect()) {
                    if (l < 32 && l > 16) {
                        context.drawGuiTexture(ModMultiplayerServerListWidget.JOIN_HIGHLIGHTED_TEXTURE, x, y, 32, 32);
                    } else {
                        context.drawGuiTexture(ModMultiplayerServerListWidget.JOIN_TEXTURE, x, y, 32, 32);
                    }
                }

                if (index > 0) {
                    if (l < 16 && m < 16) {
                        context.drawGuiTexture(ModMultiplayerServerListWidget.MOVE_UP_HIGHLIGHTED_TEXTURE, x, y, 32, 32);
                    } else {
                        context.drawGuiTexture(ModMultiplayerServerListWidget.MOVE_UP_TEXTURE, x, y, 32, 32);
                    }
                }

                if (index < this.screen.getServerList().size() - 1) {
                    if (l < 16 && m > 16) {
                        context.drawGuiTexture(ModMultiplayerServerListWidget.MOVE_DOWN_HIGHLIGHTED_TEXTURE, x, y, 32, 32);
                    } else {
                        context.drawGuiTexture(ModMultiplayerServerListWidget.MOVE_DOWN_TEXTURE, x, y, 32, 32);
                    }
                }
            }
        }
        ...
        private void update() {
            ...
            switch (this.server.getStatus()) {
                case INITIAL:
                case PINGING:
                    this.statusIconTexture = ModMultiplayerServerListWidget.PING_1_TEXTURE;
                    this.statusTooltipText = ModMultiplayerServerListWidget.PINGING_TEXT;
                    break;
                case INCOMPATIBLE:
                    this.statusIconTexture = ModMultiplayerServerListWidget.INCOMPATIBLE_TEXTURE;
                    this.statusTooltipText = ModMultiplayerServerListWidget.INCOMPATIBLE_TEXT;
                    this.playerListSummary = this.server.playerListSummary;
                    break;
                case UNREACHABLE:
                    this.statusIconTexture = ModMultiplayerServerListWidget.UNREACHABLE_TEXTURE;
                    this.statusTooltipText = ModMultiplayerServerListWidget.NO_CONNECTION_TEXT;
                    break;
                case SUCCESSFUL:
                    if (this.server.ping < 150L) {
                        this.statusIconTexture = ModMultiplayerServerListWidget.PING_5_TEXTURE;
                    } else if (this.server.ping < 300L) {
                        this.statusIconTexture = ModMultiplayerServerListWidget.PING_4_TEXTURE;
                    } else if (this.server.ping < 600L) {
                        this.statusIconTexture = ModMultiplayerServerListWidget.PING_3_TEXTURE;
                    } else if (this.server.ping < 1000L) {
                        this.statusIconTexture = ModMultiplayerServerListWidget.PING_2_TEXTURE;
                    } else {
                        this.statusIconTexture = ModMultiplayerServerListWidget.PING_1_TEXTURE;
                    }
        ...
        private boolean uploadFavicon(@Nullable byte[] bytes) {
            if (bytes == null) {
                ...
                try {
                    ...
                    ModMultiplayerServerListWidget.LOGGER.error("Invalid icon for server {} ({})", this.server.name, this.server.address, throwable);
                    return false;
                }
            }
        ...
        public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
            if (Screen.hasShiftDown()) {
                ModMultiplayerServerListWidget multiplayerServerListWidget = this.screen.serverListWidget;
        ...
        private void swapEntries(int i, int j) {
            ...
            ModMultiplayerServerListWidget.this.ensureVisible(entry);
        }

        @Override
        public boolean mouseClicked(double mouseX, double mouseY, int button) {
            double d = mouseX - ModMultiplayerServerListWidget.this.getRowLeft();
            double e = mouseY - ModMultiplayerServerListWidget.this.getRowTop(ModMultiplayerServerListWidget.this.children().indexOf(this));
            if (d <= 32.0) {
        ...
        public Text getNarration() {
            ...
            switch (this.server.getStatus()) {
                case PINGING:
                    mutableText.append(ModMultiplayerServerListWidget.PINGING_TEXT);
                    break;
                case INCOMPATIBLE:
                    mutableText.append(ModMultiplayerServerListWidget.INCOMPATIBLE_TEXT);
                    mutableText.append(ScreenTexts.SENTENCE_SEPARATOR);
                    ...
                    break;
                case UNREACHABLE:
                    mutableText.append(ModMultiplayerServerListWidget.NO_CONNECTION_TEXT);
                    break;
                default:
                    mutableText.append(ModMultiplayerServerListWidget.ONLINE_TEXT);
                    mutableText.append(ScreenTexts.SENTENCE_SEPARATOR);
